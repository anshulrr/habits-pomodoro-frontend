...
export default function UserSettingsComponent() {
    ...
    const handleScroll = function (ref) {
        ref.current.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }

    window.addEventListener("scroll", (event) => {
        if (window.scrollY > menuRef.current.offsetTop) {
    ...
    return (
        <div>
            <div ref={menuRef} className="row px-0 pb-3" style={{ position: window.scrollY > menuRef.current.offsetHeight + menuRef.current.offsetTop ? 'fixed' : 'inherit', top: "200px", display: window.innerWidth <= 992 ? "block" : "none" }}>
                <div className="col-12 px-0">
                    <div className="">

                        <button type="button" className={"btn btn-sm btn-outline-secondary py-0 px-1"} onClick={() => handleScroll(generalSettingsRef)}>
                            General Settings
                        </button>
                        <button type="button" className={"btn btn-sm btn-outline-secondary py-0 px-1"} onClick={() => handleScroll(displaySettingsRef)}>
                            Display Settings
                        </button>
                        <button type="button" className={"btn btn-sm btn-outline-secondary py-0 px-1"} onClick={() => handleScroll(statsSettingsRef)}>
                            Stats Settings
                        </button>
                    </div>
                </div>
